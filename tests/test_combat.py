...
class DummyStats(types.SimpleNamespace):
    def __init__(self, void_charge=50, focus=0, corruption_stacks=0):
...
def test_clean_strike_critical_failure(monkeypatch):
    ...
    # damage, focus_roll_base -> we set focus_roll_base such that final == 1
    ...
    assert res["corruption_gained"] == 2
...
def test_clean_strike_failed_clean_kill_becomes_consumption(monkeypatch):
    ...
    # damage, focus_roll_base -> failed clean kill (e.g., 5 < 12)
    ...
    assert res["corruption_gained"] == 1
