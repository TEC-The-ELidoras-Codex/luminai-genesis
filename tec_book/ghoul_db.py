...
class GhoulGenerator:
    ...
    def generate_batch(
        self, count: int, tier_range: tuple = (1, 3),
    ) -> list[GhoulRecord]:
        """Generate multiple Ghouls"""
        return [
            self.generate_one(tier=self.rng.randint(*tier_range)) for _ in range(count)
        ]
...
class GhoulDatabase:
    ...
    def get_random(self, tier: int | None = None) -> GhoulRecord | None:
