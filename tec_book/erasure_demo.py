...
try:
    from ghoul_db import GhoulDatabase, GhoulGenerator, create_canonical_ghouls
...
def main(args=None):
    ...
    parser.add_argument(
        "--seed", type=int, default=None, help="Seed RNG for deterministic demo",
    )
    parser.add_argument(
        "--noninteractive",
        action="store_true",
        help="Run a single non-interactive demo encounter and exit",
    )
    parser.add_argument(
        "--data-file",
        default="data/ghouls.json",
        help="Path to ghoul data file (use temp file for tests)",
    )
    ...
    while True:
        ...
        print("[2] CONSUME - Feed the Queen")
