...
class MemoryFragment:
    ...
    honored_by: str | None = None
    honored_at: str | None = None
...
class CoreStats:
    ...
    def apply_corruption(self, amount: int) -> str:
        ...
        if self.corruption_stacks >= 10:
            return "⚠️  Stage 3 Corruption"
        if self.corruption_stacks >= 7:
            return "⚠️  Stage 2 Corruption"
        if self.corruption_stacks >= 5:
...
"""
    ...
    def consume(self, ghoul: KaznakGhoul) -> str:
        ...
        self.stats.void_charge = min(
            100, self.stats.void_charge + result["void_charge"],
        )
...
def create_lumina() -> Character:
    ...
    lumina = Character(name="Lumina", archetype=Archetype.VOIDTOUCHED)
...
def create_polkin() -> Character:
    ...
    polkin = Character(name="Polkin Rishall", archetype=Archetype.TECHNOMANCER)
...
def create_elara() -> Character:
    ...
    elara = Character(name="Elara Kess", archetype=Archetype.SPLICER)
...
def create_jorin() -> Character:
    ...
    jorin = Character(name="Maya Fost", archetype=Archetype.SHADOW_OPERATIVE)
