...
def qc_file(path, expected_n=None):
    ...
    try:
        ...
        return {"filename": path.name, "error": "read_error", "error_detail": str(e)}
    # JSON parse check
    try:
        data = json.loads(txt)
        is_json = True
    ...
    return {
        ...
        "mean_score": (
            float(sum(scores) / len(scores)) if scores and len(scores) else None
        ),
        "notes": note,
    }
...
def main():
    ...
    if prompts_file.exists():
        try:
            ...
            logger.warning(
                "Failed to read prompts.yaml; falling back to heuristic count: %s", _,
            )
    ...
    under_count = [
        s
        for s in summary
        if any(n.startswith("under_count_results") for n in s.get("notes", []))
    ]
    empty_resp = [
        s
        for s in summary
        if any(n.startswith("empty_responses_indices") for n in s.get("notes", []))
    ]
    ...
    with open(out, "w") as fh:
        ...
        for s in summary:
            if (
                s.get("num_scores", 0) == 0
                or s.get("is_json") is False
                or s.get("notes")
            ):
                fh.write(
                    f"- **{s['filename']}**: notes={s.get('notes',[])}; results_count={s.get('results_count')}; num_scores={s.get('num_scores')}\n",
                )
