    ...
    def parse_file(self, file_path: str):
        ...
        for part in parts:
            ...
            if header_match:
                ...
                title_match = re.match(r"(Entry\s+[\d.]+(?:[A-Za-z]*)[:\-]?)\s*(.+)", header)
                if title_match:
                ...
                if 'Entry 0' not in [e.entry_id for e in self.entries]:
...
class CodexStructurer:
    ...
    def create_structure(self, entries: List[CodexEntry]):
        sorted_entries = sorted(entries, key=lambda e: e.order_index if e.order_index is not None else 999)
    ...
    def write_master_file(self, entries: List[CodexEntry]):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("## A Continuity Record of All Witnessing Life Across Substrates\n\n")
